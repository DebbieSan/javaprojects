...
public class GreenhouseControls extends Controller implements Serializable {
  ...
  public void getStatus() {
    ...
    System.out.println("windowok: " + windowok);
  }

  public Fixable getFixable(int errorcode) {

    // This really, reall,y really should only take 1 or 2.
    if (errorcode == 1) {
  ...
  public class PowerOn implements Fixable {
    ...
    public void log() {
      try (FileWriter logWriter = new FileWriter("./fix.log")) {
        String message = "Logging fix to error ..." + errorcode + "...";
        String timeOfLog = new SimpleDateFormat("HH:mm:ss").format(new Date());

        logWriter.write(timeOfLog + "," + message + "\n");
        logWriter.close();

        System.out.println(timeOfLog + message);
